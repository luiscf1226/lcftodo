...
export default function AppError({ error, retry }: { error: Error & { digest?: string }; retry: () => void }) {
  ...
  return (
    <div className="mx-auto mt-16 max-w-md card p-6 text-center" role="alert">
      <h1 className="text-lg font-semibold">{lostTeam ? "Ya no formas parte de este equipo" : "Ocurrió un error"}</h1>
      <p className="mt-1 text-sm text-pretty text-muted">
        {lostTeam ? "Es posible que te hayan quitado del equipo. Elige otro o crea uno nuevo." : errorMessage(error)}
      </p>
