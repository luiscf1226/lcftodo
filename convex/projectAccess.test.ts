...
async function join(
  ...
  await t.mutation(internal.memberships.applyWebhook, {
    orgId,
    userId,
    membershipId: `mem_${userId}_${seq}`,
    role,
    active: true,
    createdAt: seq,
    updatedAt: seq,
...
async function setup() {
  ...
  const apolloTodo = await a.mutation(api.todos.create, {
    projectId: apollo,
    title: "Apollo work",
    date: DAY,
...
async function exportRows(client: Ctx["a"], projectId?: Id<"projects">) {
  const now = Date.now();
  return (
    await client.query(api.activity.exportPage, {
...
describe("existing-team migration (open policy)", () => {
  test("until an admin restricts access, every member keeps seeing every project", async () => {
    ...
    const { b, c } = ctx;
    expect(await b.query(api.projectAccess.me, {})).toMatchObject({
      isAdmin: false,
      restricted: false,
      grantedProjects: null,
    });
  ...
  test("only admins manage access", async () => {
    ...
    await expect(b.mutation(api.projectAccess.grant, { projectId: apollo, userId: carol.subject })).rejects.toThrow(
      /admins/,
    );
...
describe("restricted access: two projects with different memberships", () => {
  test("reads are scoped to granted projects; admins see everything", async () => {
    ...
    expect(
      (await b.query(api.activity.list, { ...page, actorId: alice.subject })).page.every(
        (r) => r.projectName === "Apollo",
      ),
    ).toBe(true);
    ...
    const now = Date.now();
    expect(
      await b.query(api.activity.exportRange, { fromMs: now - 3_600_000, toMs: now + 3_600_000, projectId: borealis }),
    ).toEqual([]);
    ...
    expect((await a.query(api.search.todos, { query: "work" })).map((r) => r.title).sort()).toEqual([
      "Apollo work",
      "Borealis work",
    ]);
  });
  ...
  test("writes to an inaccessible project fail as if it didn't exist", async () => {
    ...
    await expect(b.mutation(api.todos.create, { projectId: borealis, title: "x", date: DAY })).rejects.toThrow(
      /Project not found/,
    );
    await expect(b.mutation(api.todos.setStatus, { todoId: borealisTodo, status: "done" })).rejects.toThrow(
      /Todo not found/,
    );
    await expect(b.mutation(api.todos.update, { todoId: borealisTodo, title: "x", date: DAY })).rejects.toThrow(
      /Todo not found/,
    );
    await expect(b.mutation(api.todos.remove, { todoId: borealisTodo })).rejects.toThrow(/Todo not found/);
    await expect(b.mutation(api.todos.carryOver, { projectId: borealis, date: DAY })).rejects.toThrow(
      /Project not found/,
    );
    await expect(
      b.mutation(api.projects.update, { projectId: borealis, name: "Mine", color: "#6366f1" }),
  ...
  test("outsiders see nothing", async () => {
    ...
    await expect(e.mutation(api.projectAccess.grant, { projectId: ctx.apollo, userId: eve.subject })).rejects.toThrow(
      /not found/,
    );
  });
});
...
describe("assignees", () => {
  test("must be active members with access to the project", async () => {
    ...
    await expect(
      a.mutation(api.todos.create, { projectId: apollo, title: "x", date: DAY, assigneeId: carol.subject }),
    ).rejects.toThrow(/access to this project/);
    await expect(
      a.mutation(api.todos.create, { projectId: apollo, title: "x", date: DAY, assigneeId: eve.subject }),
    ...
    expect((await a.query(api.projectAccess.assignable, { projectId: apollo }))?.userIds.sort()).toEqual(
      [alice.subject, bob.subject].sort(),
    );
  });
  ...
  test("after access removal: history is kept, the work leaves their list, an admin can reassign", async () => {
    ...
    expect(await a.mutation(api.projectAccess.revoke, { projectId: apollo, userId: bob.subject })).toEqual({
      openAssigned: 1,
    });
    ...
    expect(await a.query(api.todos.get, { todoId: apolloTodo })).toMatchObject({
      assigneeId: bob.subject,
      title: "Renamed",
    });
    ...
    await expect(
      a.mutation(api.todos.update, {
        todoId: next[0]._id,
        title: "Renamed",
        date: "2026-09-24",
        assigneeId: carol.subject,
      }),
...
describe("invitations", () => {
  ...
  beforeEach(() => {
    ...
    vi.spyOn(globalThis, "fetch").mockImplementation(async (input, init) => {
      ...
      if (method === "POST" && url.endsWith("/invitations")) {
        return new Response(
          JSON.stringify({ id: `inv_${++nextId}`, status: "pending", expires_at: Date.now() + 86_400_000 }),
          { status: 200 },
        );
      }
      if (method === "POST" && url.endsWith("/revoke"))
        return new Response(JSON.stringify({ id: "x", status: "revoked" }), { status: 200 });
  ...
  const grantsOf = (t: Ctx["t"], userId: string) =>
    t.run((ctx) =>
      ctx.db
        ...
        .withIndex("by_org_user", (q) => q.eq("orgId", "org_a").eq("userId", userId))
        .collect(),
    );
  ...
  test("inviting validates the caller, role and projects server-side", async () => {
    ...
    await expect(
      b.action(api.invitations.invite, { email: "x@example.com", role: "org:member", projectIds: [apollo] }),
    ).rejects.toThrow(/admins/);
    await expect(
      a.action(api.invitations.invite, { email: "x@example.com", role: "org:owner", projectIds: [apollo] }),
    ).rejects.toThrow(/Invalid role/);
    await expect(
      a.action(api.invitations.invite, { email: "x@example.com", role: "org:member", projectIds: [other] }),
    ).rejects.toThrow(/Project not found/);
    await expect(
      a.action(api.invitations.invite, { email: "not-an-email", role: "org:member", projectIds: [] }),
  ...
  test("a pending invitation's projects are granted exactly once on acceptance", async () => {
    ...
    expect(calls[0]).toMatchObject({
      method: "POST",
      body: { email_address: "dave@example.com", role: "org:member", inviter_user_id: alice.subject },
    });
    ...
    expect(
      await a.action(api.invitations.invite, { email: "dave@example.com", role: "org:member", projectIds: [borealis] }),
    ).toEqual({ outcome: "merged" });
    expect(calls).toHaveLength(1);
    const overview = await a.query(api.projectAccess.overview, {});
    expect(overview?.invitations).toMatchObject([
      { email: "dave@example.com", status: "pending", projectIds: [apollo, borealis] },
    ]);

    // Clerk: invitation accepted (with the new user id), then membership created. Retries included.
    await t.mutation(internal.invitations.applyAccepted, {
      invitationId: "inv_1",
      orgId: "org_a",
      userId: dave.subject,
    });
    ...
    expect((await a.query(api.projectAccess.overview, {}))?.invitations[0]).toMatchObject({
      status: "accepted",
      acceptedUserId: dave.subject,
    });
  ...
  test("falls back to the membership webhook's email when the accepted event isn't delivered", async () => {
    ...
    expect((await t.withIdentity(dave).query(api.todos.listForTeam, WEEK)).map((x) => x.title)).toEqual([
      "Apollo work",
    ]);
  });
  ...
  test("inviting an existing member grants projects directly", async () => {
    ...
    expect(
      await a.action(api.invitations.invite, { email: "BOB@example.com", role: "org:member", projectIds: [borealis] }),
  ...
  test("removing someone from the team ends their access immediately", async () => {
    ...
    await expect(b.mutation(api.todos.create, { projectId: ctx.apollo, title: "x", date: DAY })).rejects.toThrow(
      /signed in/,
    );
  });
  ...
  test("a membership.deleted webhook drops project grants", async () => {
    ...
    await restrict(ctx);
    const { t } = ctx;
    const row = await t.run((c) =>
      c.db
        .query("memberships")
        .withIndex("by_org_user", (q) => q.eq("orgId", "org_a").eq("userId", carol.subject))
        .unique(),
    );
    await t.mutation(internal.memberships.applyWebhook, {
      orgId: "org_a",
      userId: carol.subject,
...
describe("comments, recurring todos and the nightly carry-over", () => {
  ...
  test("recurring series respect project access and assignee eligibility", async () => {
    ...
    const standup = await a.mutation(api.recurrences.create, {
      projectId: apollo,
      startDate: DAY,
      title: "Standup",
      rule,
    ...
    const hidden = await a.mutation(api.recurrences.create, {
      projectId: borealis,
      startDate: DAY,
      title: "Borealis sync",
      rule,
    ...
    await expect(b.mutation(api.recurrences.stop, { recurrenceId: hidden, from: DAY })).rejects.toThrow(
      /Recurring todo not found/,
    );
  ...
  test("the nightly carry-over keeps running and leaves copies for people without access unassigned", async () => {
    ...
    expect(next.map((x) => [x.title, x.assigneeId])).toEqual([
      ["Apollo work", undefined],
      ["Borealis work", undefined],
    ]);
  });
});
